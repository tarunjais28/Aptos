module utils::i128 {
    ...
    /// Maximum positive magnitude for I128
    const MAX_POSITIVE_MAGNITUDE: u128 = (1 << 127) - 1;
    /// Maximum negative magnitude for I128
    const MAX_NEGATIVE_MAGNITUDE: u128 = (1 << 127);

    /// Define a struct named `I128` with copy, drop, and store attributes
    ...
    /// Function to create a new instance of `I128`
    ...
    /// Function to retrieve the sign of the number from an `I128` instance
    public fun get_is_negative(i: &I128): bool {
        // Return the sign (negative) of the number
        i.negative
    }

    /// Function to retrieve the magnitude of the number if it's positive
    public fun get_magnitude_if_positive(in: &I128): u128 {
    ...
    public fun get_magnitude_if_negative(in: &I128): u128 {
        assert!(in.negative, error::positive_value());
        // Return the magnitude of the negative number
        in.magnitude
    }

    /// Function to create an `I128` instance from a `u128` number
    public fun from_u128(from: u128): I128 {
        // Use the MSB to determine whether the number is negative or not
        let negative = (from >> 127) == 1;
        // Parse the magnitude of the number
        let magnitude = parse_magnitude(from, negative);

        // Create and return a new instance of `I128`
        new(magnitude, negative)
    }

    /// Function to multiply the magnitudes of two `I128` instances
    public fun mult_magnitude(number1: I128, number2: I128): u256 {
        // Convert the magnitudes to u256 and perform multiplication
        let num1_mag = (number1.magnitude as u256);
        let num2_mag = (number2.magnitude as u256);
        num1_mag * num2_mag
    }

    /// Helper function to parse the magnitude of the number
    fun parse_magnitude(from: u128, negative: bool): u128 {
    ...
    fun test_max_positive_magnitude() {
        ...
        // Assert that the result from `from_u128` matches the expected value
    ...
    fun test_magnitude_too_large_negative() {
        ...
        new(0x80000000000000000000000000000001, true);
    }

    // Test to ensure that an `I128` instance can be created from a positive `u128` number
    #[test]
    fun test_from_u128_positive() {
        // Assert that the result from `from_u128` matches the expected value
        assert!(from_u128(0x64673) == new(0x64673, false), 1);
    }
    ...
    fun test_single_zero_representation() {
        ...
        // Assert that instances with zero magnitude, both positive and negative, created from u128 are equal
