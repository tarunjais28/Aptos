module base_token_contract::maintainers {
    use std::signer;
    use std::vector;
    use base_token_contract::events::{emit_sub_admins_update_event, emit_admin_update_event};
    use std::option;
    use utils::error;
    ...
    public fun init_maintainers(account: &signer, addr: address) {
        assert!(!exists<Maintainers>(@base_token_contract), error::already_exists());
    ...
    public entry fun update_admin(account: &signer, new_admin: address) acquires Maintainers {
        ...
        let maintainers = borrow_global_mut<Maintainers>(@base_token_contract);
    ...
    public fun get_sub_admins(): vector<address> acquires Maintainers {
        borrow_global<Maintainers>(@base_token_contract).sub_admins
    }
    ...
    public fun get_admin(): address acquires Maintainers {
        borrow_global<Maintainers>(@base_token_contract).admin
    }
    //:!:>view functions
