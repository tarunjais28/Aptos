module multisig::maintainers {
    use std::signer;
    use std::vector;
    use multisig::events::{emit_admins_update_event, emit_validator_update_event};
    use utils::error;
    ...
    public fun init_maintainers(account: &signer, addr: address) {
        assert!(!exists<Maintainers>(@multisig), error::already_exists());
    ...
    public entry fun add_admins(account: &signer, addrs: vector<address>) acquires Maintainers {
        ...
        let maintainers = borrow_global_mut<Maintainers>(@multisig);
    ...
    public fun get_admins(): vector<address> acquires Maintainers {
        borrow_global<Maintainers>(@multisig).admins
    }
    ...
    public fun get_validators(): vector<address> acquires Maintainers {
        borrow_global<Maintainers>(@multisig).validators
    }
    //:!:>view functions
