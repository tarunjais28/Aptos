module multisig::multisig {
    ...
    use std::signer;
    use multisig::resource::{create_with_admin, get_threshold};
    use multisig::events::{emit_execute_transaction_event, emit_cast_vote_event};
    use multisig::maintainers::is_validator;
    use utils::error;
    use std::vector;
    ...
    public entry fun init(admin: &signer, threshold: u8) {
        ...
        assert!(acc_addr == @multisig, error::unauthorised_caller());
    ...
    public entry fun cast_vote(
        ...
        let votes = &mut borrow_global_mut<Votes>(@multisig).votes;
    ...
    public entry fun execute_transaction(
        ...
        // Borrow global mutable reference to Votes
        let votes = borrow_global_mut<Votes>(@multisig).votes;
    ...
    public fun get_vote(tx_hash: String): Vote acquires Votes { // Acquires Votes resource for read access
        ...
        let votes = borrow_global<Votes>(@multisig).votes;
