module interop_multisig::resource {
    use std::signer;
    use interop_multisig::events::{initialize_event_store, emit_init_event, emit_update_threshold_event};
    use interop_multisig::maintainers::{init_maintainers, is_admin};
    ...
    public entry fun update_threshold(account: &signer, threshold: u8) acquires Threshold {
        ...
        let threshold_value = borrow_global_mut<Threshold>(@interop_multisig).value;
    ...
    public fun get_threshold(): u8 acquires Threshold {
        let threshold = borrow_global<Threshold>(@interop_multisig);
        threshold.value
    }
    //:!:>view functions
