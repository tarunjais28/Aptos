#[test_only]
module interop_multisig::tests {
    use std::vector::{Self, is_empty};
    use std::unit_test;
    use std::signer;
    use interop_multisig::multisig::{init, cast_vote, get_vote, execute_transaction};
    use std::string;
    use std::option;
    ...
    use utils::error;
    use utils::i128::I128;
    use interop_multisig::maintainers::{get_admins, add_validators, get_validators};
    ...
    #[test(
        account = @interop_multisig
    )]
    entry fun test_init(account: &signer) {
        aptos_framework::account::create_account_for_test(@interop_multisig);
        init(account, 1);
    }
